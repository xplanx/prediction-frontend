...
export function getBscRpcUrls(): IChainRpcUrl {
  const urls: IChainRpcUrl = { default: { http: [BSC_RPC_ENDPOINTS[0]] } };

  for (let i = 1; i < BSC_RPC_ENDPOINTS.length; i++) {
    urls[i] = { http: [BSC_RPC_ENDPOINTS[i]] };
  }
